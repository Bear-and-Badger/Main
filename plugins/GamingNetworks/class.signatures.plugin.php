...
// Define the plugin:
$PluginInfo['GamingNetworks'] = array(
   'Name' => 'Gaming Networks',
   'Description' => 'This plugin allows users to attach their IDs from gaming networks to their profiles.',
   'Version' => '0.1',
...
class GamingNetworksPlugin extends Gdn_Plugin {
   
   public function Base_GetAppSettingsMenuItems_Handler(&$Sender) {
      $Menu = &$Sender->EventArguments['SideMenu'];
      $Menu->AddItem('Forum', T('Forum'));
      $Menu->AddLink('Forum', T('Gamingnetwork'), 'settings/networks', 'Garden.Settings.Manage');
   }
   ...
   public function SettingsController_Signatures_Create($Sender) {
      ...
      $Sender->Title('Gaming Networks');
      $Sender->AddSideMenu('settings/networks');
      $Sender->Render('plugins/GamingNetworks/views/settings.php');
   }
   ...
   public function SettingsController_ToggleSignatures_Create($Sender) {
      ...
      if (Gdn::Session()->ValidateTransientKey(GetValue(0, $Sender->RequestArgs)))
         SaveToConfig('Plugins.GamingNetworks.Enabled', C('Plugins.GamingNetworks.Enabled') ? FALSE : TRUE);
         
      Redirect('settings/networks');
   }

   public function ProfileController_AfterAddSideMenu_Handler(&$Sender) {
      if (!C('Plugins.GamingNetworks.Enabled'))
         return;
      ...
      if ($Sender->User->UserID == $ViewingUserID) {
         $SideMenu->AddLink('Options', T('Gaming network Settings'), '/profile/networks', FALSE, array('class' => 'Popup'));
      } else {
         $SideMenu->AddLink('Options', T('Gaming network Settings'), '/profile/networks/'.$Sender->User->UserID.'/'.Gdn_Format::Url($Sender->User->Name), 'Garden.Users.Edit', array('class' => 'Popup'));
      }
   }
   ...
   public function ProfileController_Signature_Create(&$Sender) {
      ...
      $ConfigArray = array(
         'Plugin.GamingNetworks.Live'	  => NULL,
         'Plugin.GamingNetworks.PSN'      => NULL,
         'Plugin.GamingNetworks.Steam'    => NULL
      );
      ...
      $Sender->SetData('Plugin-GamingNetworks-ForceEditing', ($SigUserID == Gdn::Session()->UserID) ? FALSE : $Sender->User->Name);
...
*/
      ...
      $Sender->SetData('Plugin-GamingNetworks-UserSignatures', $UserSignatures);
   }
...
*/
   ...
   protected function _DrawSignature(&$Sender) {
      ...
      if (isset($UserSignatures[$SourceUserID])) {
         $HideImages = ArrayValue('Plugin.GamingNetworks.HideImages', $Sender->Data('Plugin-Signatures-ViewingUserData'), FALSE);
   ...
   protected function _HideAllSignatures(&$Sender) {
      ...
      $HideSigs = ArrayValue('Plugin.GamingNetworks.HideAll', $Sender->Data('Plugin-Signatures-ViewingUserData'), FALSE);
